...
public class MDL_NEServlet extends HttpServlet {
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		for (int i = 0; i < LL; i++) {
			...
			(new Random()).nextInt(1) > 0 ? "q3" : "dex", 
			i % 100, "node" + i, "ChengDu" + i, 
			genNEType(),
			(new Random()).nextInt(1) > 0 ? "sm" : "mc",
			(new Random()).nextInt(1) > 0 ? "4.0" : "4.1",
			(new Random()).nextInt(1) > 0 ? "2015-01-22 22:15:09" : "2015-01-25 13:39:22",
			(new Random()).nextInt(1) > 0 ? "10.105.3.11" : "10.105.3.12",
			"N/A", 
			(new Random()).nextInt(1) > 0 ? "suppervised" : "unsuppervised",
			(new Random()).nextInt(1) > 0 ? "available" : "unavailable", 
			(new Random()).nextInt(1) > 0 ? "critical" : "major"));
		}
